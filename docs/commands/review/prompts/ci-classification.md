...
The prompt asks each reviewer agent to decide whether every failed check is caused by the pull request changes or is likely flaky, external, or infrastructure-related. Classifications are majority-voted per check.
...
The review command runs this prompt before reviewer agents review, after required check results are fetched and at least one required check failed.
