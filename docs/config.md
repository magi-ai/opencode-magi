...
| `triage.kind.bug.type`                | Both    | No                           | `["Bug"]`                                           | Issue types that classify an issue as a bug and skip Kind Vote when issue types are available through GraphQL.             |
| `triage.kind.feature.label`           | Both    | No                           | `["enhancement"]`                                   | Labels that classify an issue as a feature request and skip Kind Vote.                                                     |
| `triage.kind.feature.type`            | Both    | No                           | `["Feature"]`                                       | Issue types that classify an issue as a feature request and skip Kind Vote when issue types are available through GraphQL. |
...
| `triage.prompts.*`                    | Both    | No                           | [md](/src/prompts/templates/triage)                 | Prompt template overrides for triage phases.                                                                               |
