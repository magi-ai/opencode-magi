...
Object values are deep merged. Array values are replaced, so setting `review.reviewers` in the project config replaces the global reviewer list.
...
Agent model fields accept either a single `provider/model` string or an ordered candidate array. Candidate arrays are validated from the first entry, and the first valid candidate is applied.
...
| `review.reviewers[].account`           | Both    | Yes                              | -                                                    | GitHub account used to post reviews and approvals.                                                                      |
...
| `review.checks`                        | Both    | No                               | -                                                    | Required PR check waiting, exclusion, and retry configuration for review gating.                                        |
...
| `review.checks.wait`                   | Both    | No                               | `true`                                               | Whether to wait for required PR checks before review. Optional checks do not gate review.                               |
...
| `triage.signals`                       | Both    | No                               | `[]`                                                 | Optional user-defined signal IDs and descriptions evaluated after the final triage disposition and category are known.  |
| `triage.automation.close`              | Both    | No                               | `false`                                              | Whether rejected, invalid, or duplicate issues are closed after an author-mentioned close comment.                      |
...
| `triage.automation.label`              | Both    | No                               | default rules                                        | Conditional add/remove label rules matched by disposition, category, and signals. Set `[]` to disable label automation. |
...
| `merge.editor.model`                   | Both    | Yes (`/magi:merge`)              | -                                                    | Single `provider/model` string or ordered candidate array used by the editor.                                           |
...
| `merge.checks`                         | Both    | No                               | -                                                    | Required PR check waiting configuration after editor changes.                                                           |
| `merge.checks.wait`                    | Both    | No                               | `true`                                               | Whether to wait for required PR checks after the editor pushes a fix commit. Optional checks do not gate post-edit CI.  |
...
| `merge.automation.conflict`            | Both    | No                               | `false`                                              | Whether `/magi:merge` attempts one editor-driven merge queue conflict recovery after a queued PR is dequeued.           |
...
Magi passes resolved OpenCode permission rules to every child session. `agents.permissions` is the common base for all Magi agents, and `review.reviewers[].permissions` or `merge.editor.permissions` override that base for one agent.
